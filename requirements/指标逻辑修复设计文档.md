...
| 19 | INDX_0064 | 代发薪客户净增 | 活动/任务客户范围内 [开始日,跑批日] 新增代发薪签约客户数，按(CUST_ID,PERSN_LEGAL_BK_CODE)去重；签约 SIGN_TYPE IN('321','322') 且摘要含工资 | ECIF_T02_A_CUST_SIGN_REL + CBS_KDPL_ZHMINX + CBS_KCDA_PZJCXX | ✅ 已实现 |
...
> 注（v1.8）：0064 原列入占位并阻塞于 `DWD_CUST_SIGN_CTRAKT.CTRAKT_TYP` 6 种子类型值域（Q4 v1.3.1）。2026-08-24 口径改为直连 ECIF/CBS（SIGN_TYPE 321/322 + 摘要含工资，见记忆卡片 v1.8），已在 plan_006 7.17/7.18 实现，故从占位移除。
