...
ETL 过程中使用临时表时需遵循以下规则：

| 类型 | 前缀 | 创建方式 | 使用场景 | 生命周期 |
|------|------|----------|----------|----------|
| 会话临时表 | `temp_` | `CREATE TEMP TABLE` | SQL 脚本、单步 ETL 任务 | 会话结束自动删除 |
| 物理临时表 | `TMP_` | `CREATE TABLE IF NOT EXISTS` | 存储过程中的多步骤处理 | 需手动清理 |

**注意**：ETL 脚本（`.sql` 文件）中推荐使用会话临时表（`temp_`），存储过程中使用物理临时表（`TMP_`）。
