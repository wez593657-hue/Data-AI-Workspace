...
| TX_DATE | 首次交易日期 | VARCHAR2(8) | YBT_POLICY_BASE_INFO | c.ACCEPT_DATE | 趸交满一年起算基准(YYYYMMDD) |
| LAST_TX_DATE | 最近交易日期 | VARCHAR2(8) | YBT_POLICY_FEE_LIST | a.TX_DATE(ODS字段待核对) | 保单最近一笔交易日期 |
...
| ACTL_TERM_DATE | 实际终止日期 | VARCHAR2(8) | YBT_POLICY_FEE_LIST | 状态交易最新日期 | 状态交易(2/3/4/5/6/8)最新日期→CONT_STATUS终止变更日→CANCL_INSUR_DATE回退 |
...
| INSUR_AMT | 当前保险金额 | NUMBER(20,2) | YBT_POLICY_FEE_LIST | a.ORD_AMT | = 新单保费+续期保费累计；终止/未生效/趸交满一年/期缴缴满/宽限期过60天未缴 均置0 |
| NEW_INSUR_AMT | 首期保费(新单保费) | NUMBER(20,2) | YBT_POLICY_FEE_LIST | a.ORD_AMT | TRAN_TYPE='0' 时的 ORD_AMT；支持指标0061与ADS.INSUR_FRST_PREM_AMT |
| POLICY_STATE | 保单状态 | VARCHAR2(8) NOT NULL | YBT_POLICY_BASE_INFO | c.CONT_STATUS | 0未生效/1正常/2失效；唯一状态判定源 |
