...
AS
  ...
  V_PRC_NAME             VARCHAR(64)  := 'PRC_ADS_CUST_SLEEP_WAKE_STATIS';
...
BEGIN
  ...
  DELETE FROM ADS_CUST_SLEEP_WAKE_STATIS S
   WHERE (S.STATIS_CYCLE = 'M' AND S.DATA_DATE = TO_CHAR(LAST_DAY(ADD_MONTHS(TO_DATE(V_SYSDAT, 'YYYYMMDD'), -1)), 'YYYYMMDD'))
      ...
      OR (S.STATIS_CYCLE = 'Y' AND S.DATA_DATE = TO_CHAR(TRUNC(TO_DATE(V_SYSDAT, 'YYYYMMDD'), 'YYYY') - 1, 'YYYYMMDD'));
  ...
  SELECT D.PERSN_LEGAL_BK_CODE,
           ...
           START WITH O.ORG_ID IN (
                 SELECT DISTINCT D1.ORG_ID
                   ...
                   WHERE D1.DATA_DATE = V_SYSDAT
                      ...
                      OR (D1.STATIS_CYCLE = 'Y' AND D1.DATA_DATE = TO_CHAR(TRUNC(TO_DATE(V_SYSDAT, 'YYYYMMDD'), 'YYYY') - 1, 'YYYYMMDD'))
   ...
   WHERE D.DATA_DATE = V_SYSDAT
      ...
      OR (D.STATIS_CYCLE = 'Y' AND D.DATA_DATE = TO_CHAR(TRUNC(TO_DATE(V_SYSDAT, 'YYYYMMDD'), 'YYYY') - 1, 'YYYYMMDD'))
