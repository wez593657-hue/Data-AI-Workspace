CREATE OR REPLACE PROCEDURE PRC_ADS_CUST_POTN_UPGRADE_STATIS(
    V_SYSDAT IN VARCHAR,
    OUTCDE   OUT INTEGER
)
AS
...
BEGIN
  ...
  SELECT D.PERSN_LEGAL_BK_CODE,
         ...
         CASE
           WHEN (D.LVL_CRIT = '03' AND NVL(M.AUM_BAL, 0) >= 50000)
             ...
             OR (D.LVL_CRIT = '05' AND NVL(M.AUM_BAL, 0) >= 500000)
             OR (D.LVL_CRIT = '06' AND NVL(M.AUM_BAL, 0) >= 1000000)
             OR (D.LVL_CRIT = '07' AND NVL(M.AUM_BAL, 0) >= 3000000)
           THEN '1'
           ELSE '0'
         END,
         D.QUAL_STATE,
  ...
  SELECT S.PERSN_LEGAL_BK_CODE,
         ...
         SUM(CASE WHEN S.PNT_QUAL_STATE = '1' THEN 1 ELSE 0 END),
         CASE WHEN COUNT(*) = 0 THEN 0 ELSE ROUND(SUM(CASE WHEN S.PNT_QUAL_STATE = '1' THEN 1 ELSE 0 END) / COUNT(*) * 100, 2) END,
