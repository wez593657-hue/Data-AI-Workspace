...
BEGIN
  ...
  SELECT a.PERSN_LEGAL_BK_CODE,                                             -- 法人行号_取资产快照的法人行号
         ...
         CASE
           -- v2.3.0: 当月新客展示'未评级'
           -- 判断条件: OPEN_DATE所在月份 = 跑批日所在月份
           WHEN TRUNC(TO_DATE(REPLACE(SUBSTR(c.OPEN_DATE, 1, 10), '-', ''), 'YYYYMMDD'), 'MM') = V_BN_MONTH
           THEN '未评级'
           -- 非当月新客取DWS_CUST_LVL_INFO的客户等级，无记录默认'未评级'
           ELSE NVL(l.CUST_LVL, '未评级')
         END,                                                                -- 客户等级
         CASE
           -- v2.1.0: 新客周期左闭右开
           -- 周期1: 开户0~30天（不含30天）
         ...
         m.ORG_ID                                                             -- v2.3.4: 归属机构_取自理财管户经理所在机构
