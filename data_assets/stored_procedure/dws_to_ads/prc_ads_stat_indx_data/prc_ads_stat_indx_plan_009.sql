...
(
    ...
    V_PRC_DESC VARCHAR2(100) := '指标数据统计步骤99处理完成 9';
		...
		MBK_BASE AS
		 (
			...
			SELECT SC.PATH_CODE,
			 ...
			 INNER JOIN MBK_CUST_INFO MI ON MI.CUST_CORE_NO = SC.CUST_ID
		...
		MBK_TRANS_ALL AS
		 (
			...
			SELECT B.PATH_CODE,
			 ...
			 INNER JOIN IBP_IB_LIST_PLAT Y ON Y.ACCT_NO = CA.ACCT
																		AND Y.ITEM_ID = '100015'
																		AND Y.CHANNEL_ID = '3031'
																		AND Y.PLAT_DATE BETWEEN B.TERM_BEGIN_DATE AND
																				V_SYSDAT
			...
			SELECT B.PATH_CODE,
						 ...
						 1
				FROM MBK_BASE B
			 INNER JOIN MBK_CUST_LOG_TRAN T ON T.CUST_NO = B.CUST_NO
																		 AND T.TRAN_DATE BETWEEN
																				 B.TERM_BEGIN_DATE AND V_SYSDAT
																		 AND T.TRAN_STATUS = '1'
																		 AND NVL(T.TRAN_AMT, 0) <> 0
			UNION ALL
			-- 理财
			SELECT B.PATH_CODE,
						 B.STATIS_CALIB,
						 ...
						 1
				FROM MBK_BASE B
			 INNER JOIN MBK_CUST_LOG_FINANCE F ON F.CUST_NO = B.CUST_NO
																				AND F.TRAN_DATE BETWEEN
																						B.TERM_BEGIN_DATE AND V_SYSDAT
																				AND F.TRAN_STATUS = '1'
			...
			SELECT B.PATH_CODE,
						 ...
						 B.DATA_BLNG,
						 B.PERSN_LEGAL_BK_CODE,
						 CAST(NULLIF(F.TRAN_AMT, '') AS DECIMAL(31, 2)),
						 1
				FROM MBK_BASE B
			 INNER JOIN MBK_CUST_LOG_FEE F ON F.CUST_NO = B.CUST_NO
																		AND F.TRAN_DATE BETWEEN B.TERM_BEGIN_DATE AND
																				V_SYSDAT
																		AND F.TRAN_STATUS = '1'
																		AND NVL(CAST(NULLIF(F.TRAN_AMT, '') AS
																								 DECIMAL(31, 2)),
																						0) <> 0
			...
			SELECT B.PATH_CODE,
						 ...
						 1
				FROM MBK_BASE B
			 INNER JOIN MBK_CUST_LOG_OPER O ON O.CUST_NO = B.CUST_NO
																		 AND O.TRAN_CODE = 'atm/noCardPredMoney'
																		 AND O.TRAN_STATUS = '1'
																		 AND O.TRAN_DATE BETWEEN
																				 B.TERM_BEGIN_DATE AND V_SYSDAT
			UNION ALL
			-- 扫码取款
			SELECT B.PATH_CODE,
						 ...
						 1
				FROM MBK_BASE B
			 INNER JOIN MBK_MKP_PROCESS_INFO P ON P.CUST_NO = B.CUST_NO
																				AND P.TRANS_SN NOT LIKE 'AP%'
																				AND LENGTH(P.TRANS_SN) > 18
																				AND P.SENCE_TIME BETWEEN
																						B.TERM_BEGIN_DATE AND V_SYSDAT
			UNION ALL
			-- 存款/乐惠存
			SELECT B.PATH_CODE,
						 ...
						 1
				FROM MBK_BASE B
			 INNER JOIN MBK_CUST_LOG_BASE_FINANCE B2 ON B2.CUST_NO = B.CUST_NO
																							AND B2.TRAN_STATUS = '1'
																							AND B2.TRAN_DATE BETWEEN
																									B.TERM_BEGIN_DATE AND
																									V_SYSDAT
			...
			SELECT B.PATH_CODE,
						 ...
						 1
				FROM MBK_BASE B
			 INNER JOIN MBK_CUST_LOG_LOAN L ON L.CUST_NO = B.CUST_NO
																		 AND L.TRAN_DATE BETWEEN
																				 B.TERM_BEGIN_DATE AND V_SYSDAT
																		 AND NVL(CAST(NULLIF(L.BUSINESSSUM, '') AS
																									DECIMAL(31, 2)),
																						 0) <> 0
			UNION ALL
			-- 移动支付
			SELECT B.PATH_CODE,
						 ...
						 1
				FROM MBK_BASE B
			 INNER JOIN MBK_QR_C2B_QUERYTRANS Q ON Q.CUST_NO = B.CUST_NO
																				 AND Q.STATUS = '1'
																				 AND Q.ORDERTIME BETWEEN
																						 B.TERM_BEGIN_DATE AND V_SYSDAT
																				 AND NVL(CAST(NULLIF(Q.TXN_AMT, '') AS
																											DECIMAL(31, 2)),
																								 0) <> 0)
		SELECT PATH_CODE,
					 V_SYSDAT,
