CREATE OR REPLACE PROCEDURE crmdm.prc_ads_stat_indx_aum_balance(
...
BEGIN
    ...
    SELECT sm.path_code,
           ...
           SUM(CASE WHEN b.data_date = v_sysdat       AND b.bal_type = '1' THEN NVL(b.aum_bal, 0) ELSE 0 END) AS curnt_aum,
           SUM(CASE WHEN b.data_date = V_YAR_BEGIN    AND b.bal_type = '1' THEN NVL(b.aum_bal, 0) ELSE 0 END) AS yr_begin_aum,
           SUM(CASE WHEN b.data_date = V_MTH_END      AND b.bal_type = '1' THEN NVL(b.aum_bal, 0) ELSE 0 END) AS mth_end_aum,
           SUM(CASE WHEN b.data_date = V_QRT_END      AND b.bal_type = '1' THEN NVL(b.aum_bal, 0) ELSE 0 END) AS qrt_end_aum,
           SUM(CASE WHEN b.data_date = v_sysdat       AND b.bal_type = '4' THEN NVL(b.aum_bal, 0) ELSE 0 END) AS curnt_yr_avg_aum,
           SUM(CASE WHEN b.data_date = V_YAR_PREV_END AND b.bal_type = '4' THEN NVL(b.aum_bal, 0) ELSE 0 END) AS prev_yr_avg_aum,
           SUM(CASE WHEN b.data_date = v_sysdat       AND b.bal_type = '2' THEN NVL(b.aum_bal, 0) ELSE 0 END) AS curnt_mth_avg_aum,
           SUM(CASE WHEN b.data_date = V_MTH_END      AND b.bal_type = '2' THEN NVL(b.aum_bal, 0) ELSE 0 END) AS prev_mth_avg_aum
    ...
    SELECT 'A', v_sysdat, s.data_blng, s.statis_dim, '营销活动', 'INDX_0047',
     ...
     INNER JOIN TMP_STAT_INDX_BAL_AGGR b
      ...
      LEFT JOIN crm_sys_post p
        ON s.blng_type = 'M' AND p.post_id = s.blng_id AND p.job_cls = 'C'
      LEFT JOIN DEPO_VALUE_INIT v
        ON v.org_id               = CASE WHEN s.blng_type = 'O' THEN s.blng_id ELSE p.org_id END
...
END prc_ads_stat_indx_aum_balance;