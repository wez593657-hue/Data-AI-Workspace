    ...
    INSERT INTO TMP_STAT_INDX_AGGR_006
        ...
        SELECT SM.PATH_CODE,
         ...
         INNER JOIN DWD_ACCT_INSUR I ON I.CUST_ID = SM.CUST_ID
              ...
              AND I.TX_DATE BETWEEN SM.TERM_BEGIN_DATE AND V_SYSDAT
    ...
    INSERT INTO TMP_STAT_INDX_AGGR_006
        ...
        SELECT SM.PATH_CODE,
         ...
         INNER JOIN ADS_CRM_R_CUST_LABLE L ON L.CUST_ID = SM.CUST_ID
              AND L.PERSN_LEGAL_BK_CODE = SM.PERSN_LEGAL_BK_CODE
    ...
    INSERT INTO TMP_STAT_INDX_AGGR_006
        ...
        WITH SCOPE_ALL AS
         (SELECT 'A' AS PATH_CODE,
                 ...
                 M.MCT_ID
            FROM TMP_STAT_INDX_SCOPE S
           INNER JOIN UEPP_PAY_MCT_INFO M ON ((S.BLNG_TYPE = 'O' AND M.ORG_ID = S.BLNG_ID) OR
    ...
    INSERT INTO TMP_STAT_INDX_AGGR_006
        ...
        WITH BIND_CARD AS
         (SELECT DISTINCT SGN_ACCT_ID_DE AS CARD_NO                          -- 已绑卡卡号集合
            FROM ECPP_E_TXN_SIGN
           WHERE STATUS = '00'
             AND INSTG_ID IN ('Z2004944000010', 'Z2007933000010', 'Z2009331000015')),
        SCOPE_ALL AS
         (SELECT 'A' AS PATH_CODE,
                 '营销活动' AS STATIS_CALIB,
                 S.STATIS_DIM,
        ...
        ELIGIBLE AS
         (SELECT N.*
           ...
           INNER JOIN DWD_CUST_INDV_INFO I ON I.CUST_ID = N.CUST_ID
                AND I.PERSN_LEGAL_BK_CODE = N.PERSN_LEGAL_BK_CODE
