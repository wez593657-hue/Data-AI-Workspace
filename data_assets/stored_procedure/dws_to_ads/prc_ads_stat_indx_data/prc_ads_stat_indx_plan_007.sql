...
(
    ...
    V_PRC_DESC VARCHAR2(100) := '指标数据统计步骤77处理完成 7';
    V_PRC_NAME VARCHAR2(32) := 'PRC_ADS_STAT_INDX_PLAN_007';
...
BEGIN
		 ...
		 (SELECT SM.PATH_CODE,
				...
				LEFT JOIN DWD_CUST_INDV_INFO CI ON CI.CUST_ID = SM.CUST_ID
																			 AND CI.PERSN_LEGAL_BK_CODE =
																					 SM.PERSN_LEGAL_BK_CODE
				LEFT JOIN MBK_CUST_INFO MI ON MI.CUST_CORE_NO = SM.CUST_ID
	...
	INSERT INTO TMP_STAT_INDX_AGGR_007
		...
		SELECT SM.PATH_CODE,
					 ...
					 SM.STATIS_DIM,
					 SM.STATIS_CALIB,
					 'INDX_0083',COUNT(DISTINCT C.KAHAOOOO),
                     0,
                     SM.PERSN_LEGAL_BK_CODE
            FROM (SELECT DISTINCT PATH_CODE,
                                        STATIS_CALIB,
                                        ...
                                        PERSN_LEGAL_BK_CODE
                    FROM SCOPE_ALL) SM
           INNER JOIN CBS_KCDA_PZJCXX C ON C.KEHUHAOO = SM.CUST_ID
                                        AND CASE
