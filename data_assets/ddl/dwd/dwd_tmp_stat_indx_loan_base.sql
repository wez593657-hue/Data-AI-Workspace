...
DROP TABLE crmdm.TMP_STAT_INDX_LOAN_BASE;
CREATE TABLE crmdm.TMP_STAT_INDX_LOAN_BASE (
    PATH_CODE            VARCHAR(2)   NOT NULL,  -- 路径码: 08=营销活动 09=目标任务
    STATIS_CALIB         VARCHAR(64)  NOT NULL,  -- 统计口径: 活动ID/任务ID
    ...
    LOAN_BAL             NUMBER(20,2) NULL,      -- 期初贷款余额(DWS_CUST_ASSE_LIAB(_HIS).LOAN_BAL, BAL_TYPE=1)
    CLASS_FIVE            VARCHAR(2)   NULL,      -- 贷款五级分类: 1正常/2关注/3次级/4可疑/5损失
...
COMMENT ON COLUMN TMP_STAT_INDX_LOAN_BASE.STATIS_CALIB        IS '统计口径: 活动ID/任务ID';
...
COMMENT ON COLUMN TMP_STAT_INDX_LOAN_BASE.LOAN_BAL            IS '贷款期末余额 来源DWS_CUST_ASSE_LIAB(_HIS).LOAN_BAL, BAL_TYPE=1';
COMMENT ON COLUMN TMP_STAT_INDX_LOAN_BASE.CLASS_FIVE           IS '贷款五级分类: 1正常 2关注 3次级 4可疑 5损失';
