...
CREATE TABLE crmdm.dwd_acct_insur (
	...
	insur_amt numeric(20, 2) NULL, -- 当前保险金额(新单+续期累计;终止/缴费期满/宽限期过=0)
	policy_state varchar(8) NOT NULL, -- 保单状态: 0未生效/1正常/2失效
	tx_typ varchar(1) NULL, -- 交易类型(统一置空,不消费)
	persn_legal_bk_code varchar(4) NULL, -- 法人行号
...
COMMENT ON COLUMN crmdm.dwd_acct_insur.tx_date IS '首次交易日期';
COMMENT ON COLUMN crmdm.dwd_acct_insur.last_tx_date IS '最近交易日期';
...
COMMENT ON COLUMN crmdm.dwd_acct_insur.cancl_insur_date IS '保险期间结束日期(推算值,仅参考)';
COMMENT ON COLUMN crmdm.dwd_acct_insur.actl_term_date IS '实际终止日期';
...
COMMENT ON COLUMN crmdm.dwd_acct_insur.insur_amt IS '当前保险金额';
COMMENT ON COLUMN crmdm.dwd_acct_insur.new_insur_amt IS '首期保费(新单保费)';
COMMENT ON COLUMN crmdm.dwd_acct_insur.policy_state IS '保单状态: 0未生效/1正常/2失效';
...
CREATE TABLE crmdm.tmp_dwd_acct_insur_snap (
	...
	prdkt_name varchar(100) NULL,
	prdkt_cate_big varchar(64) NULL,
	insur_bid_form_no varchar(40) NOT NULL,
