# ADS数据字典 - ADS_CUST_POTN_UPGRADE_CUST_DTL

## 表信息

| 属性 | 值 |
| --- | --- |
| 层级 | ADS - 应用数据层 |
| 表名 | ADS_CUST_POTN_UPGRADE_CUST_DTL |
| 中文名称 | 潜力提升客户明细列表 |
| 来源模型 | ADS应用层数据模型_CRM_ V1.0.xlsx / 潜力提升客户明细列表 |
| 更新时间 | 2026-07-20 |

## 字段列表

| 字段名 | 字段中文说明 | 数据类型 | 长度 | 是否为空 | 默认值 | 主键 | 外键 | 枚举说明 | 业务含义 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| PERSN_LEGAL_BK_CODE | 法人行号 | VARCHAR2 | 4 | 【待确认】 | - | - | - | - | 法人行号 |
| DATA_DATE | 数据日期 | VARCHAR2 | 8 | 【待确认】 | - | - | - | - | 数据日期 |
| CUST_ID | 客户编号 | VARCHAR2 | 20 | 【待确认】 | - | - | - | - | 客户编号 |
| CUST_NAME | 客户名称 | VARCHAR2 | 100 | 【待确认】 | - | - | - | - | 客户名称 |
| CUST_LVL | 客户等级 | VARCHAR2 | 2 | 【待确认】 | - | - | - | - | 客户等级 |
| LVL_CRIT | 临界等级 | VARCHAR2 | 2 | 【待确认】 | - | - | - | - | 临界等级 |
| DEPO_CURNT_DEPO_BAL | 活期余额 | NUMBER | 20,2 | 【待确认】 | - | - | - | - | 活期余额 |
| FIXD_DEPO_BAL | 定期余额 | NUMBER | 20,2 | 【待确认】 | - | - | - | - | 定期余额 |
| FIN_AMT | 理财余额 | NUMBER | 20,2 | 【待确认】 | - | - | - | - | 理财余额 |
| CNTCT_STATE | 接触状态 | VARCHAR2 | 1 | 【待确认】 | - | - | - | - | 接触状态 |
| QUAL_STATE | 时点达标状态 | VARCHAR2 | 1 | 【待确认】 | - | - | - | 0否，1是 | T-1日时点AUM达到临界目标等级 |
| POST_ID | 管户经理 | VARCHAR2 | 20 | 【待确认】 | - | - | - | - | 管户经理 |
| ORG_ID | 归属机构 | VARCHAR2 | 7 | 【待确认】 | - | - | - | - | 归属机构 |
| STATIS_CYCLE | 统计周期(月/季/年) | VARCHAR2 | 2 | 【待确认】 | - | - | - | - | 统计周期(月/季/年) |

---

*数据字典版本: v1.0 | 生成时间: 2026-07-20*
