-- ============================================================
-- 结构差异说明（2026-08-25）：
-- 本脚本为单体验证参照版，全程串行执行，共用单一 TMP_STAT_INDX_AGGR。
-- 生产环境已拆分为 plan_003~009 各自写入专属表 TMP_STAT_INDX_AGGR_003~_009，
-- 并由 plan_010 合并至 TMP_STAT_INDX_AGGR_010 后统一强校验+落库（并行跑批隔离）。
-- 两者指标口径等价；分表仅影响并行隔离与清理方式，不影响计算结果。
-- ============================================================
