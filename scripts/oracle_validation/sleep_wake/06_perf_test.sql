...
BEGIN
  FOR rec IN (SELECT 10000 AS n FROM dual UNION ALL
    ...
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ADS_CUST_SLEEP_WAKE_DTL';
    EXECUTE IMMEDIATE 'TRUNCATE TABLE ADS_CUST_SLEEP_WAKE_STATIS';
    ...
    EXECUTE IMMEDIATE
      'INSERT INTO DWD_CUST_INDV_INFO(CUST_ID, CUST_NAME, CUST_TYP, PERSN_LEGAL_BK_CODE, ORG_LEAD)
       SELECT ''PERF''||LPAD(LEVEL,8,''0''), ''C''||LEVEL, ''1'', ''BK10'',
              CASE MOD(LEVEL,2) WHEN 0 THEN ''ORG101'' ELSE ''ORG100'' END
    ...
    EXECUTE IMMEDIATE
      'INSERT INTO DWD_CUST_MAN(CUST_ID, MNGR_POST_ID, ORG_ID, MNG_TYP, VALID_DATE, PERSN_LEGAL_BK_CODE)
       SELECT ''PERF''||LPAD(LEVEL,8,''0''), ''PM''||MOD(LEVEL,2),
              CASE MOD(LEVEL,2) WHEN 0 THEN ''ORG101'' ELSE ''ORG100'' END,
              ''1'', ''20260702'', ''BK10''
         FROM dual CONNECT BY LEVEL <= ' || rec.n;
    EXECUTE IMMEDIATE
      'INSERT INTO DWS_CUST_ASSE_LIAB(DATA_DATE, PERSN_LEGAL_BK_CODE, CUST_ID, ORG_ID, BAL_TYPE, AUM_BAL, DEPO_CURNT_DEPO_BAL, FIXD_DEPO_BAL, FIN_BAL, INSUR_BAL)
       SELECT ''20260702'', ''BK10'', ''PERF''||LPAD(LEVEL,8,''0''),
              CASE MOD(LEVEL,2) WHEN 0 THEN ''ORG101'' ELSE ''ORG100'' END,
              ''1'', MOD(LEVEL,100), 50, 0, 0, 0
         FROM dual CONNECT BY LEVEL <= ' || rec.n;
    -- baseline snapshot for wake compare
    EXECUTE IMMEDIATE
      'INSERT INTO DWS_CUST_ASSE_LIAB(DATA_DATE, PERSN_LEGAL_BK_CODE, CUST_ID, ORG_ID, BAL_TYPE, AUM_BAL, DEPO_CURNT_DEPO_BAL, FIXD_DEPO_BAL, FIN_BAL, INSUR_BAL)
       SELECT ''20260701'', ''BK10'', ''PERF''||LPAD(LEVEL,8,''0''),
              CASE MOD(LEVEL,2) WHEN 0 THEN ''ORG101'' ELSE ''ORG100'' END,
              ''1'', MOD(LEVEL,100), 50, 0, 0, 0
         FROM dual CONNECT BY LEVEL <= ' || rec.n;

    v_t1 := DBMS_UTILITY.GET_TIME;
    PRC_ADS_CUST_SLEEP_WAKE_DTL(v_batch, v_rc);
    v_t2 := DBMS_UTILITY.GET_TIME;
    v_dtl := (v_t2 - v_t1) / 100;

    v_t1 := DBMS_UTILITY.GET_TIME;
    PRC_ADS_CUST_SLEEP_WAKE_STATIS(v_batch, v_rc);
    v_t2 := DBMS_UTILITY.GET_TIME;
    v_stat := (v_t2 - v_t1) / 100;

    SELECT COUNT(*) INTO v_d0 FROM ADS_CUST_SLEEP_WAKE_DTL;
    SELECT COUNT(*) INTO v_s0 FROM ADS_CUST_SLEEP_WAKE_STATIS;
