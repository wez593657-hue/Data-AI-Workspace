...
DECLARE
  ...
  BEGIN
    BEGIN
      IF p_proc = 'DTL' THEN
        PRC_ADS_CUST_SLEEP_WAKE_DTL(p_val, v_rc);
      ELSE
        PRC_ADS_CUST_SLEEP_WAKE_STATIS(p_val, v_rc);
      END IF;
...
DECLARE
  v_rc NUMBER;
BEGIN
  PRC_ADS_CUST_SLEEP_WAKE_DTL('20260702', v_rc);
  DBMS_OUTPUT.PUT_LINE('DTL valid rc='||v_rc);
  PRC_ADS_CUST_SLEEP_WAKE_STATIS('20260702', v_rc);
...
SELECT DATA_DATE, PRC_NAME, STEP_NO, LOG_FLG, SUBSTR(LOG_MSG,1,70) AS LOG_MSG
 ...
 WHERE PRC_NAME LIKE 'PRC_ADS_CUST_SLEEP%' AND LOG_FLG < 0
   AND ROWNUM <= 3
