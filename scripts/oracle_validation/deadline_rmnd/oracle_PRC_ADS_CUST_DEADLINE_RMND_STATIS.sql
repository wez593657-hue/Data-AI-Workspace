-- DROP PROCEDURE PRC_ADS_CUST_DEADLINE_RMND_ST(in varchar, out int4);

CREATE OR REPLACE PROCEDURE PRC_ADS_CUST_DEADLINE_RMND_ST(v_sysdat varchar, outcde OUT integer)
AS 
  ------------------------------------------------------------------
  -- 存储过程名称: 到期承接统计表处理
  -- 存储过程编号: PRC_ADS_CUST_DEADLINE_RMND_ST
  -- 处理周期: 日
  ...
  V_PRC_NAME VARCHAR(64) := 'PRC_ADS_CUST_DEADLINE_RMND_ST';
  ...
  V_CURR_MONTH_END VARCHAR2(8);
  V_CURR_QUARTER_END VARCHAR2(8);
  V_CURR_YEAR_END VARCHAR2(8);
...
BEGIN
  ...
  DELETE FROM ADS_CUST_DEADLINE_RMND_STATIS
   WHERE (STATIS_CYCLE = 'M' AND DATA_DATE = V_CURR_MONTH_END)
      ...
      OR (STATIS_CYCLE = 'Y' AND DATA_DATE = V_PREV_YEAR_END);
  ...
  INSERT INTO TMP_CDR_STAT_BASE (
      ...
      FIXED_MATURE_TRAN_FIN_AMT, FIXED_FIN_TRAN_INSUR_AMT, FIN_MATURE_TRAN_FIXED_AMT,
  ...
  SELECT d.PERSN_LEGAL_BK_CODE,
         ...
         NVL(d.FIXED_FIN_TRAN_INSUR_AMT, 0),
   ...
   WHERE (d.STAT_PERD = 'M' AND d.DATA_DATE IN (V_CURR_MONTH_END, V_PREV_MONTH_END))
      OR (d.STAT_PERD = 'Q' AND d.DATA_DATE IN (V_CURR_QUARTER_END, V_PREV_QUARTER_END))
      OR (d.STAT_PERD = 'Y' AND d.DATA_DATE IN (V_CURR_YEAR_END, V_PREV_YEAR_END));
  COMMIT;
  ...
  SELECT b.PERSN_LEGAL_BK_CODE,
         ...
         b.FIXED_MATURE_TRAN_FIN_AMT, b.FIXED_FIN_TRAN_INSUR_AMT, b.FIN_MATURE_TRAN_FIXED_AMT,
  ...
  SELECT s.PERSN_LEGAL_BK_CODE,
         ...
         s.STAT_PERD,
         s.STATIS_TYP,
         COUNT(DISTINCT CASE WHEN s.EXPR_AMT > 0 THEN s.CUST_ID || CHR(1) || s.PERSN_LEGAL_BK_CODE || CHR(1) || s.ORG_ID END),
         COUNT(DISTINCT CASE WHEN s.MATURE_TTL_AMT > 0 THEN s.CUST_ID || CHR(1) || s.PERSN_LEGAL_BK_CODE || CHR(1) || s.ORG_ID END),
         SUM(s.EXPR_AMT),
         ...
         END,
         CASE WHEN s.STATIS_TYP = '2' THEN 0                                   -- 存款转理财：限定定期存款（STATIS_TYP=1）
              WHEN s.STATIS_TYP = '0' THEN ROUND(
                   (SELECT NVL(SUM(x.FIXED_MATURE_TRAN_FIN_AMT), 0) FROM TMP_CDR_STAT_SRC x
                   ...
                   / NULLIF((SELECT SUM(x.EXPR_AMT) FROM TMP_CDR_STAT_SRC x
                     WHERE x.PERSN_LEGAL_BK_CODE = s.PERSN_LEGAL_BK_CODE
                       ...
                       AND x.STATIS_OBJ = s.STATIS_OBJ
                       AND x.STAT_PERD = s.STAT_PERD
                       AND x.STATIS_TYP = '1'), 0) * 100, 2)
              ELSE ROUND(SUM(s.FIXED_MATURE_TRAN_FIN_AMT) / NULLIF(SUM(s.EXPR_AMT), 0) * 100, 2)
         END,
         CASE WHEN s.STATIS_TYP IN ('1', '2') THEN 0                           -- 保险转化率按全部口径展示（覆盖STATIS_TYP 1和2）
              ELSE ROUND(SUM(s.FIXED_FIN_TRAN_INSUR_AMT) / NULLIF(SUM(s.EXPR_AMT), 0) * 100, 2)
         END,
         CASE WHEN s.STATIS_TYP = '1' THEN 0                                   -- 理财转存款（理财转定期）：限定理财（STATIS_TYP=2）
              WHEN s.STATIS_TYP = '0' THEN ROUND(
                   (SELECT NVL(SUM(x.FIN_MATURE_TRAN_FIXED_AMT), 0) FROM TMP_CDR_STAT_SRC x
                     WHERE x.PERSN_LEGAL_BK_CODE = s.PERSN_LEGAL_BK_CODE
                       AND x.DATA_DATE = s.DATA_DATE
                       AND x.STATIS_OBJ = s.STATIS_OBJ
                       AND x.STAT_PERD = s.STAT_PERD
                   ...
                   / NULLIF((SELECT SUM(x.EXPR_AMT) FROM TMP_CDR_STAT_SRC x
                     WHERE x.PERSN_LEGAL_BK_CODE = s.PERSN_LEGAL_BK_CODE
                       ...
                       AND x.STATIS_OBJ = s.STATIS_OBJ
                       AND x.STAT_PERD = s.STAT_PERD
                       AND x.STATIS_TYP = '2'), 0) * 100, 2)
              ELSE ROUND(SUM(s.FIN_MATURE_TRAN_FIXED_AMT) / NULLIF(SUM(s.EXPR_AMT), 0) * 100, 2)
         END
