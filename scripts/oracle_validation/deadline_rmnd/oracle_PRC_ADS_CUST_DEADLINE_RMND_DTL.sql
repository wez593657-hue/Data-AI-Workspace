-- DROP PROCEDURE prc_ads_cust_deadline_rmnd_dtl(in varchar, out int4);

CREATE OR REPLACE PROCEDURE prc_ads_cust_deadline_rmnd_dtl(v_sysdat varchar, outcde OUT integer)
AS 
  ------------------------------------------------------------------
  -- 存储过程名称: 到期承接明细表处理
...
BEGIN
  ...
  TRUNC_TMP('TMP_CDR_DTL_CROSS_CONV');
  ...
  SELECT c.CUST_ID                                           AS CUST_ID,             -- 客户编号
         ...
         C.PERSN_LEGAL_BK_CODE                                         AS ORG_ID,              -- 归属机构
  ...
  INSERT INTO ADS_CUST_DEADLINE_RMND_DTL (
      ...
      CNTCT_STATE,                       -- 接触状态
      UNDTAKE_STATE,                     -- 承接状态
      FIXED_FIN_TRAN_INSUR_AMT,   -- 到期转保险金额
  ...
  SELECT
      ...
      w.STAT_PERD                                                              AS STAT_PERD,                         -- 统计周期：M-月,Q-季,Y-年
      w.STATIS_TYP                                                             AS STATIS_TYP,                        -- 承接类型：0-全部,1-存款,2-理财
                 ...
                 SELECT 1
                  ...
                  WHERE m.CUST_ID = w.CUST_ID
                    ...
                    AND TO_DATE(REPLACE(SUBSTR(m.MKT_TIME, 1, 10), '-', ''), 'yyyymmdd')
                        BETWEEN w.FIRST_EXPR_DT AND w.TAKE_END_DT_30D         -- 30天承接窗口内（口径30）
                    -- DEFECT-005 修复(2026-08-01): 接触状态增加跑批日截止(口径21/27)
                    AND TO_DATE(REPLACE(SUBSTR(m.MKT_TIME, 1, 10), '-', ''), 'yyyymmdd')
                        <= TO_DATE(V_SYSDAT, 'yyyymmdd')                      -- 营销时间不得晚于跑批日 V_SYSDAT
             )
            THEN '1' ELSE '0'
      ...
      NVL(t.BUY_INSUR_AMT_30D, 0)                                              AS FIXED_FIN_TRAN_INSUR_AMT,   -- 30天到期转保险金额
      NVL(cv.FIN_MATURE_TRAN_FIXED_AMT, 0)                                    AS FIN_MATURE_TRAN_FIXED_AMT,         -- 理财到期30天转定期金额(预聚合,按客户+机构维度)
      NVL(cv.FIXED_MATURE_TRAN_FIN_AMT, 0)                                    AS FIXED_MATURE_TRAN_FIN_AMT,         -- 定期到期30天转理财金额(预聚合,按客户+机构维度)
    ...
    FROM TMP_CDR_DTL_DUE_WIN w
    -- DEFECT-006 修复(2026-08-01): TAKE_AMT 按统计周期实例(BGN_DT/END_DT)内联计算
    LEFT JOIN (
          SELECT w2.STAT_PERD,
                 w2.BGN_DT,
                 w2.END_DT,
    ...
    LEFT JOIN TMP_CDR_DTL_AUM_BAL ap
     ...
     -- DEFECT-007 修复(2026-08-01): 关联到期前一日AUM时补 DATA_DATE,避免同 STAT_PERD 多周期实例行重复
     AND ap.DATA_DATE = TO_CHAR(w.FIRST_EXPR_DT - 1, 'yyyymmdd')
     -- DEFECT-006 修复(2026-08-01): CROSS_CONV 按统计周期实例(BGN_DT/END_DT)内联计算
     LEFT JOIN (
           SELECT ta.STAT_PERD,
                  ta.BGN_DT,
                  ta.END_DT,
